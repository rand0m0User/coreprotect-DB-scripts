ATTACH DATABASE '<insert the database you wish to merge with the currently open file here>' AS db2;

--add all "new" players to co_user
INSERT OR IGNORE INTO co_user (time, user, uuid) SELECT time, user, uuid FROM db2.co_user
...
INSERT OR IGNORE INTO co_art_map (art) SELECT art FROM db2.co_art_map
...
INSERT OR IGNORE INTO co_world (world) SELECT world FROM db2.co_world
WHERE NOT EXISTS (SELECT 1 FROM co_world WHERE co_world.world = db2.co_world.world);

--merge any missing blockdata entries
INSERT OR IGNORE INTO co_blockdata_map (data) SELECT data FROM db2.co_blockdata_map
...
INSERT OR IGNORE INTO co_entity_map (entity) SELECT entity FROM db2.co_entity_map
...
INSERT INTO co_chat (time,user,wid,x,y,z,message) SELECT time, 
	(SELECT id FROM co_user WHERE user = (SELECT user FROM db2.co_user WHERE db2.co_user.id = db2.co_chat.user)) as user,
	(SELECT wid FROM co_world WHERE world = (SELECT world FROM db2.co_world WHERE db2.co_world.world = db2.co_world.world)) as wid,
	x, y, z, message
	FROM db2.co_chat;
...
INSERT INTO co_command (time,user,wid,x,y,z,message) SELECT time, 
	(SELECT id FROM co_user WHERE user = (SELECT user FROM db2.co_user WHERE db2.co_user.id = db2.co_command.user)) as user,
	(SELECT wid FROM co_world WHERE world = (SELECT world FROM db2.co_world WHERE db2.co_world.world = db2.co_world.world)) as wid,
	x, y, z, message
	FROM db2.co_command;
...
INSERT INTO co_sign (time,user,wid,x,y,z,action,color,color_secondary,data,waxed,face,line_1,line_2,line_3,line_4,line_5,line_6,line_7,line_8) SELECT time, 
	(SELECT id FROM co_user WHERE user = (SELECT user FROM db2.co_user WHERE db2.co_user.id = db2.co_sign.user)) as user,
	(SELECT wid FROM co_world WHERE world = (SELECT world FROM db2.co_world WHERE db2.co_world.world = db2.co_world.world)) as wid,
...
INSERT INTO co_item (time,user,wid,x,y,z,type,data,amount,action,rolled_back) SELECT time, 
	(SELECT id FROM co_user WHERE user = (SELECT user FROM db2.co_user WHERE db2.co_user.id = db2.co_item.user)) as user,
	(SELECT wid FROM co_world WHERE world = (SELECT world FROM db2.co_world WHERE db2.co_world.world = db2.co_world.world)) as wid,
	x, y, z,
	(SELECT id FROM co_material_map WHERE material = (SELECT material FROM db2.co_material_map WHERE db2.co_material_map.id = db2.co_item.type)) as type,
...
INSERT INTO co_block (time,user,wid,x,y,z,type,data,meta,blockdata,action,rolled_back) SELECT time, 
	(SELECT id FROM co_user WHERE user = (SELECT user FROM db2.co_user WHERE db2.co_user.id = db2.co_block.user)) as user,
	(SELECT wid FROM co_world WHERE world = (SELECT world FROM db2.co_world WHERE db2.co_world.world = db2.co_world.world)) as wid,
	x, y, z,
	(SELECT id FROM co_material_map WHERE material = (SELECT material FROM db2.co_material_map WHERE db2.co_material_map.id = db2.co_block.type)) as type,
	(SELECT id FROM co_blockdata_map WHERE data = (SELECT data FROM db2.co_blockdata_map WHERE db2.co_blockdata_map.id = db2.co_block.data)) as type,
...
INSERT INTO co_container (time,user,wid,x,y,z,type,data,amount,metadata,action,rolled_back) SELECT time, 
	(SELECT id FROM co_user WHERE user = (SELECT user FROM db2.co_user WHERE db2.co_user.id = db2.co_container.user)) as user,
	(SELECT wid FROM co_world WHERE world = (SELECT world FROM db2.co_world WHERE db2.co_world.world = db2.co_world.world)) as wid,
	x, y, z,
	(SELECT id FROM co_material_map WHERE material = (SELECT material FROM db2.co_material_map WHERE db2.co_material_map.id = db2.co_container.type)) as type,
		data, amount, metadata, action, rolled_back
	FROM db2.co_container;
